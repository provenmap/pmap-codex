...
> **Distribution mirror — built artifacts only (codex plugins v0.7.11).** Generated from a private source repository. Do not open pull requests here.

# ProvenMap Plugins — ProvenMap Code, ProvenMap Work & ProvenMap Architect
...
| **ProvenMap Work** | `pmap-work` | Developers / writers | A document set — specs, RFCs, ADRs, policies, wikis → a **knowledge board** | `/analyze-docs` |
...
ProvenMap Code and ProvenMap Work **produce** boards from raw material and emit the same node/edge wire model;
ProvenMap Architect **works over** the boards they produce (any board — code boards included). Per‑plugin
docs: **[ProvenMap Code](content/code/README.md)** · **[ProvenMap Work](content/work/README.md)** ·
...
| Host | ProvenMap Code | ProvenMap Work | ProvenMap Architect | How you install |
| ---- | :------: | :------: | :------------: | --------------- |
...
> [!NOTE]
> **Cursor ships ProvenMap Code only** for now — ProvenMap Work and ProvenMap Architect on Cursor are planned,
...
The commands below are live. `provenmap/pmap-claude` and `provenmap/pmap-codex` both publish
**v0.7.2**, and the `provenmap` marketplace serves `pmap-code`, `pmap-work` and `pmap-arch`
from it.
...
/plugin install pmap-work@provenmap        # documents → knowledge board
...
Then install **pmap-code**, **pmap-work**, and/or **pmap-arch** from `/plugins` and restart
...
### ProvenMap Code & ProvenMap Work — project binding
...
## Usage — the two‑phase workflow (ProvenMap Code & ProvenMap Work)
...
3. **`/analyze`** (ProvenMap Code) or **`/analyze-docs`** (ProvenMap Work) — _Phase 2_: the full analysis.
...
| `/analyze-docs` (same flags) | ProvenMap Work | _Phase 2_ — analyze the document set (incremental) |
