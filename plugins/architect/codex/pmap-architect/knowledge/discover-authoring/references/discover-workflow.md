...
- an **insight** brief's output — Code / Connect:
  `node <cli> --save-insight <output> --board-slug <entry board> --require-pack --push --host codex --domain architect` (the entry board is the payload's `boardSlug`); Architect:
