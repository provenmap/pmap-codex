...
| `handoff` | Where does `x` hand off to the rest of the system? | a drill-down board with at least one boundary port whose parent-board edge is drawn (one per board, ranked by crossings) | the inner element with most port crossings → per branch, its own edge onto the port, then an ascent from the port onto the parent-board node it stands for; `measurement` = crossing edges | strength when that element makes every crossing (a single seam); observation otherwise |
...
A trail crosses layers only where the graph proves the crossing, in the shapes the server
accepts: a **descent** leaves the node that drills into the board and lands on an element of
that board; an **ascent** leaves a drill-down and lands on the node that owns it — or, from a
boundary port (`port--<parent node>`), on the node the port stands for. A port is reached only
by its own edge inside the drill-down and left only by that ascent: the element's edge onto the
port, then up onto the parent-board node (journey, handoff), or a descent onto the element that
holds the port edge (chokepoint, cascade). The parent-board edge the port rolled up into belongs
to every element of the drill-down and is never the trail's hop. The menu marks such a trail `crosses layers`; on the
