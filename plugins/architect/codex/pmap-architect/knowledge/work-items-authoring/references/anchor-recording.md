...
<!-- Mirrored from platform's web Work item Editor so MCP-authored work items render native
     and read the same to implementers:
       apps/web/src/features-ui/work-board/work-item-editor/work-item-editor-verbs.ts (questionFor)
       apps/web/src/redux/store/slices/artifacts/work-item-builder/v2/materialize-work-item.ts
...
later code push _proves_ the element went away). Over MCP you cannot generate that claim on a
directive work item — for a pure removal, prefer making the actual board edit (`delete_nodes` /
`delete_edges`; committing the session generates a `board_diff` work item with the concrete, provable
