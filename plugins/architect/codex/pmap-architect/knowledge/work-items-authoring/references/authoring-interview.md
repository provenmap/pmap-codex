...
Dimensions and question templates for the work item grill — the round that turns "we should do X"
into a work item a developer can implement without coming back to ask. Pick 2–4 per round driven
...
- "Open work item `<work-item-slug>` already touches `<elements>` — extend it, supersede it, or keep
  both?" (Genuine decision point: AskUserQuestion.)
- "`<work-item-slug>` is sitting in `needs_clarification` on the same elements — does this
