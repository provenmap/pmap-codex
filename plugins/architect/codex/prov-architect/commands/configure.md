...
reads the token from the `PROV_MCP_TOKEN` environment variable — configuration is: mint a token,
...
```bash
export PROV_MCP_TOKEN="ck_mcp_live_…"        # the minted token
# optional, self-hosted/dev platforms only:
export PROV_MCP_URL="https://<your-platform>/api/mcp"
```
...
token is wrong/revoked: back to Step 1. Unreachable → check `PROV_MCP_URL` and network, re-run
`/status`.
