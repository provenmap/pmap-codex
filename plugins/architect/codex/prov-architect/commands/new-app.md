---
category: author
description: "Author · WRITE-CAPABLE: Plan a new system on the existing landscape — grill, place it, sketch its target architecture, draft its founding spec"
...
**architect-core**, and **board-reading**; the founding spec uses **specs-authoring**.
...
One write session (recorded via `--session open`). `create_nodes` on root (the node + proposed
edges, correct archetype + container; `--validate diagram` first) →
`create_board(ownerNodeSlug, newBoardSlug, name)`. Repo-backed systems are placed as **root
landscape nodes only** (app-nesting rule); if this is an extraction from an existing app, say
so: new node + board here, re-wiring intents there.
...
reality disagrees. Commit the session; close the ledger entry.

### Step 4 — close the binding gate, then stage the founding spec

Draft the founding spec in-session (full specs-authoring quality, kept in the drafts file),
...
- Planning material is a document → `bind_reference_source` on the new board (enough to
  author).
- Architect defers → the classic narration: *"Specs and intents need a code-bound board —
  bind the repo, then rerun `/author-spec <board>` and I'll stage this draft."*

Once authorable: `create_spec` (with requirement `anchors[]` grounding the L1 sketch) → the
intents offer (specs-authoring step 5).
