...
- `--rollup --apply` **does the edge merge itself** — it folds every resolved pair into the board — new pairs added as `uses`; existing edges keep their `type` and your `detailedDescription` while weight, class, provenance and the fact `description` are refreshed; stale rollup-backed `uses` pairs removed; model-only edges (no `metadata.provenance`) untouched — and re-runs the integrity gates before writing. Never hand-merge `.provenmap/skeletons/<board-slug>.edges.json`; it is a diagnostic copy. On exit 3 nothing was written: fix the board named by `boardPath` per `gate.errors[]` and re-run. It rewrites the board file, so **re-read the board JSON before your next edit** — never write back a copy held from before the apply. Afterwards: reclassify an edge's `type` where reading the involved files shows the real relation (`reads_from`, `writes_to`, `sync_call`, `async_message`, `data_flow`, … — server edge archetype names only; the archetype is what gives the edge its arrowhead) and put your reasoning in `detailedDescription` — the next apply keeps both (see "Edge ownership" below) — and add semantic edges the rollup cannot see.
...
- After writing the board JSON, author this board's styling plan (Step 8.4 of /analyze: signals →
  ...
  validation failure delete the plan and continue unstyled). Read
  `${PLUGIN_ROOT}/knowledge/board-styling/SKILL.md` before authoring it — a layer board is styled
  to the same doctrine as L0: composition, size and icons do the work, semantic tokens are the
  exception. A coverage or saturation warning from the validator exits 0 but means the plan
  over-styled: cut the excess tokens and re-validate. The orchestrator's /sync applies it.
