...
**Print every `display` verbatim** — never reformat, reorder or summarise; branch only on exit codes and named fields. Capture (steps 1–4) needs no credentials — never require `/configure` or `/login` first; only steps 5–7 talk to the server.
...
**2 Poll** — the user is picking; wait: `node ${PLUGIN_ROOT}/scripts/pmap-inspect.js --poll --max-wait 240`. `selectStatus: "pending"` → poll again (while the user is still picking) · `"complete"`/`"browser_closed"` with selections → go to 3 · `"cancelled"`, or `browser_closed` with nothing picked → stop · exit 3 → browser connection lost; relay `error`, offer `--start` again.
