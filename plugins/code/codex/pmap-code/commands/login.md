...
Connect this codebase to ProvenMap by signing in through the browser. The
...
```bash
node ${PLUGIN_ROOT}/scripts/pmap-login.js --start --host codex --domain code --plugin-version 0.10.11   # add --rebind only for /login switch
# add --base-url <url> when $ARGUMENTS contains an http(s) URL
```
