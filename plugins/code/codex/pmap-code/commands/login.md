...
```bash
node ${PLUGIN_ROOT}/scripts/pmap-login.js --start --host codex --domain code --plugin-version 0.24.0   # add --rebind only for /login switch
# add --base-url <url> when the user's argument is an http(s) URL
```
...
On `status: "complete"`, close with the Outcome — the brief carries this codebase's
real state, so the next move is a fact, not a guess:

**Outcome:** `node ${PLUGIN_ROOT}/scripts/pmap-status.js --brief --domain code --command login` → Done · Left · Next, per `${PLUGIN_ROOT}/knowledge/outcome/SKILL.md`.
