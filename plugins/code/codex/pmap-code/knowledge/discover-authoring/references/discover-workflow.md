...
occur: the Code and Connect CLI takes `--domain code` (and `--host codex` on a push)
...
```bash
# Code / Connect
node <cli> --briefs <ids|recommended> --rules ${PLUGIN_ROOT}/knowledge/discover-authoring/references/authoring.md [--board-slug <slug>] [--plan <path>] --domain code
# Architect
...
node <cli> --report --domain code   # Code / Connect
```
