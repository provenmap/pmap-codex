...
description: How /ground decides which board nodes are substantiated by which repo documents — the citation quality bar, drift handling, and per-genre reading priority. Use during /ground's evidence-proposal step, or whenever judging if a document backs a node's claim. Defines what counts as substantiation, anchor/excerpt discipline, and how to treat drifted or unlinked evidence.
metadata:
  author: ProvenMap
  version: 0.1.0
---
...
`/ground` calls this skill in its evidence-proposal step, after `--pull` mirrors the board (or, in a repo `/analyze` built and `/sync` pushed, reads the local board) and inventories the corpus. The CLI already did the mechanical part (mirroring, hashing, diffing); this skill owns the judgment call underneath it — deciding **which documents substantiate which nodes**.
...
3. **Propose links for `unlinkedNodes`** — for each node slug with zero evidence, search
   ...
   `/ground`'s closing report as a finding, not a failure — name the node and say why, because the
...
A `--pull` that reports nothing drifted, missing or unlinked is a quiet run: `/ground` closes on the
Outcome without proposing anything, which is what makes a scheduled drift check cheap — see
`${PLUGIN_ROOT}/knowledge/provenmap-integration/references/recurring-runs.md` for the surfaces.
