...
- **Switch to a different board (browser)** — re-bind this project to another board
  ...
  1. Run `node ${PLUGIN_ROOT}/scripts/pmap-login.js --start --rebind --host codex --domain code --plugin-version 0.11.0` and print the JSON `display` field verbatim in your reply — the Bash output panel is collapsed for the user (the browser opens best-effort).
