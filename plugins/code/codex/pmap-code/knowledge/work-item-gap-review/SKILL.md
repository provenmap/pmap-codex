---
name: work-item-gap-review
user-invokable: false
description: Review an architect-authored work item against current code reality before implementing it, then classify the gap as clean, minor, or blocking. Used by /work-items Step 4.5 to decide whether to implement, or bounce the work item back to the architect for clarification. Provides the gap-table method, the blocking criteria, and how to draft the bounce-back note.
metadata:
  author: ProvenMap
  version: 0.1.0
---

# Work item Gap Review — Methodology

A work item is the architect's picture of a change; the code is the reality the developer lives in. Between the moment the architect authored the work item and the moment you implement it, the two can drift. This skill runs **before any edit** (`/work-items` Step 4.5): it decides whether the work item is safe to implement now, or whether the gap is the architect's to resolve.
...
For **each** piece of the work item's spec, find its counterpart in the code and record whether they agree. Cover every one that applies:

| Spec element | Where it lives on the work item | What to check in the code |
| --- | --- | --- |
| Anchors | `workItem.anchors[]` / the `--show` anchor→file map | Does each anchor still resolve to a real file/element? |
| Directive | `workItem.directive` | Are the components/behaviours it names actually present and shaped as described? |
| Proposed changes | `board_diff` `suggestions[]` | Is each add/remove/modify still applicable, or already done / impossible? |
| Attachments | downloaded files under `.provenmap/work-items/attachments/<id>/` | Do they still match the contracts/shape of the code? |
...
2. **expected → found bullets** — one per gap: what the work item assumed, and what the code actually shows.
3. **The decision you need** — what the architect must change or confirm for the work item to become implementable.

Show the draft to the user for approval before sending. Then the command runs
`pmap-work-items.js --clarify <workItemId> --note "<approved note>"`. Never send an empty or auto-written note.
