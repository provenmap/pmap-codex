...
Classify the work item as **blocking** — do not implement, take it to the user — when **any** of these hold:
...
2. **The work item is stale.** `stale: true` — the server detected the anchor facts drifted from the baseline the architect authored against. Staleness alone is blocking unless the drift is provably irrelevant to this change.
...
6. **Most anchors are unresolved.** More than ~half of the work item's anchors fail to resolve to real code — the work item's frame of reference has moved out from under it.
