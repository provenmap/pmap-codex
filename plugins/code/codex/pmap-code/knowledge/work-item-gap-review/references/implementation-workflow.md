...
2. Follow `workItem.directive` as the spec. For `board_diff` payloads, realize each suggestion in code (e.g. an `add` node suggestion means introducing that component; a `remove` edge suggestion means severing that dependency)
3. Respect the project's conventions — match existing style, keep the change scoped to the work item
