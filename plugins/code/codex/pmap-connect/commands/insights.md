...
**0 Prerequisites** — `boardSlug` from `.provenmap/config.json` is the **primary board**. Then `node ${PLUGIN_ROOT}/scripts/pmap-insights.js --list-insight-skills --board-slug <boardSlug> --kind knowledge --domain connect`. Exit 2 → stop: "No board data found — run `/sync` first"; `featureAvailable: false` → stop: "No insight skills available for this account"; empty `skills` → stop: "No insight skills configured — contact your workspace admin". A failed check stops the run. On success `skills[]` is a precis: slug, name, category, description, duration.
...
**Outcome:** `node ${PLUGIN_ROOT}/scripts/pmap-status.js --brief --domain connect --command insights` → Done · Left · Next, per `${PLUGIN_ROOT}/knowledge/outcome/SKILL.md`.
