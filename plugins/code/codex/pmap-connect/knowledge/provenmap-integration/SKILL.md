...
- **Switch to a different board (browser)** — re-bind this project to another board
  ...
  1. Run `node ${PLUGIN_ROOT}/scripts/pmap-login.js --start --rebind --host codex --domain connect --plugin-version 0.25.3` and print the JSON `display` field verbatim in your reply — the Bash output panel is collapsed for the user (the browser opens best-effort).
