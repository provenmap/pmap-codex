...
**This is the plugin's first write-capable command.** Every other ProvenMap command only reads the document set and writes `.provenmap/` state; this one modifies project files to implement an intent. Never edit files before the user has picked an intent and you have claimed it, and always show the user what you changed.
