...
The `prov-code` plugin — codebase architecture analysis for [ProvenMap Portal](https://provenmap.com). Discovers your project's components, classifies them against your org's archetype catalogue, maps relationships, and syncs the result as a layered architecture board you can review and share.

Prov Code runs in **Codex**. Its sibling, **Prov Work** (`prov-work`), does the same for a document set → a knowledge board. Both emit the same node/edge wire model and sync through the same engine.
...
Then install **prov-code** from `/plugins` and restart Codex.

> `prov-code` is the install id — everywhere else it's **Prov Code**.
