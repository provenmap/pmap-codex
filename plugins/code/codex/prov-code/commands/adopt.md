...
The primary argument is `--aspect <kind>` (this is the flag `prov-adopt.js` itself takes). `--db`/`--api` are back-compat shorthand for the two original kinds, kept for muscle memory — they resolve to the same `--aspect database.schema` / `--aspect api.surface` underneath.
...
If the synced analysis is missing, **stop** and tell the user to run `/sync` first — do not fabricate slugs. `prov-adopt.js` enforces this too and exits with a "no synced spine" error, but catch it early with a clear message.

`prov-adopt.js` also refuses (exit 1) on a **branch mismatch** — the binding is pinned to one branch and the server rejects pushes from any other. Relay its `error` field verbatim; it names the pinned branch and the fix.
...
4. **Adopt it**:
   ```
   node ${PLUGIN_ROOT}/scripts/prov-adopt.js --aspect <kind> --payload <payload-file> --mode <mode>
   ```
...
`prov-adopt.js` prints an `AspectUpsertResult` (or, with `--dry-run`, just the validated payload's slug cross-check). Summarise it for the user:
