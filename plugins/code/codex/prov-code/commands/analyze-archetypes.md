...
- **Connect now** → run the browser login here, printing each JSON `display` verbatim **in your reply** (the Bash output panel is collapsed for the user): `node ${PLUGIN_ROOT}/scripts/prov-login.js --start`, then `node ${PLUGIN_ROOT}/scripts/prov-login.js --poll --analyze-cmd analyze` (generous Bash timeout, e.g. 250s). On `status: "complete"`, continue; anything else — stop, the display explains.
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-precondition.js --branch-only
```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-archetypes.js --no-cache --kind code
```
...
> **Lock-shape note:** in all three branches the lock must include `commitHash` and `catalogueHash` (from Step 1) so [prov-precondition.js](../scripts/prov-precondition.js) can detect when subsequent /analyze runs are still aligned with this scan. `skippedAt` is always non-null in these branches — that field is what makes the precondition re-prompt.
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-propose-archetypes.js \
...
`.provenmap/archetype-analysis.lock.json` is the single source of truth used by [prov-precondition.js](../scripts/prov-precondition.js) (invoked by `/analyze` Step -1) and `/status` (display). Fields:
