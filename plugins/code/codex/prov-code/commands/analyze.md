...
1. Run the precondition script:
   ...
   ```bash
   node ${PLUGIN_ROOT}/scripts/prov-precondition.js --kind code
   ```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-prepass.js --coverage
```
...
1. Run the archetypes CLI to fetch available archetypes from the server:
   ...
   ```bash
   node ${PLUGIN_ROOT}/scripts/prov-archetypes.js --kind code
   ```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-prepass.js --out .provenmap/skeletons/repo.json --summary
```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-prepass.js --scope-path <parent-node-path> --out .provenmap/skeletons/<board-slug>.json --summary
```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-prepass.js --board-report <board-slug>
```
