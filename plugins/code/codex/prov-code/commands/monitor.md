...
```bash
node ${PLUGIN_ROOT}/scripts/prov-insights.js --list-insight-skills
```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-insights.js --correlate .provenmap/monitoring/signals.json --out .provenmap/monitoring/skeleton.json
```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-insights.js --save-insight .provenmap/monitoring/skeleton.json --board-slug <boardSlug> --require-pack --push
```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-insights.js --save-insight .provenmap/monitoring/skeleton.json --board-slug <boardSlug> --require-pack --push --propose-intents
```
...
4. Scheduling — follow [references/scheduling.md](../knowledge/monitoring-correlation/references/scheduling.md): if this host can create schedules from the session (a `/schedule`-style skill for cloud routines, or the desktop app's scheduled tasks), offer to create a recurring "`run /monitor`" at the chosen cadence now (the user confirms). Otherwise print the copy-paste setup block from that reference. For cloud/unattended runs, note that credentials go in the run environment as `PROV_BINDING_TOKEN` / `PROV_API_SECRET`, and correlation uses `--from-server`.
