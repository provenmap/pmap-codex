...
- **Scope before steps.** Copy each node you cite from `pack.elements` into `scope.elements` once, keeping its pre-assigned `key`/`board` verbatim. Paths/findings reference keys, never raw slugs. (The pack comes from `prov-insights.js --build-context --demo` — see [the command](../../commands/demo-insights.md) Step 3.)
