...
| Intents | Explicit architect work items with anchors | `prov-intents.js` (pack `intents.open[]`) |
...
- The bundle is composed on the platform from three tiers — PROV defaults → org customizations → **app-tier overrides**. Where instructions conflict, the app tier wins.
...
- Skill files are **lock-managed by `/skills`** (`prov-skills.lock.json`). Never edit them as part of a build — a local edit blocks future syncs of that file. If a spec is wrong, the fix belongs on the platform.
