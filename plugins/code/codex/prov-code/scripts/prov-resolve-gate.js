 ...
 * Denies `prov-intents.js --resolve <id> --kind implemented` unless fresh
...
function main() {
  ...
  const command =
    input && input.tool_input && typeof input.tool_input.command === "string"
      ...
      : "";
  if (!command.includes("prov-intents.js")) return;
  ...
  console.log(
    JSON.stringify({
      hookSpecificOutput: {
        ...
        permissionDecisionReason: `Resolving as implemented needs fresh passing verify evidence — ${problem}. Run the project's checks and record each one first: prov-intents.js --record-verify ${idMatch[1]} --command "<cmd>" --exit-code <n>`,
      },
    }),
  );
}
