...
```bash
node ${PLUGIN_ROOT}/scripts/prov-insights.js --list-insight-skills --board-slug <boardSlug> --kind knowledge
```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-insights.js --get-insight-skill <skill-slug>
```
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-insights.js --build-context --board-slug <boardSlug> --out .provenmap/insights/<boardSlug>.context.json --summary
```
...
- **Exit 1/2 or any error** → fall back to reading `.provenmap/boards/<boardSlug>.json` (+ `manifest.json` + `layerBoardSlug` children) directly and mint keys/aliases by hand, per `references/execution-protocol.md`.
...
11. **Write to temp file** and save/push. **First ensure the pack exists for `{{primaryBoardSlug}}`** — if you switched the primary board away from the one you built the pack for in Step 1.6, rebuild it now (`--build-context --board-slug <primaryBoardSlug> --out .provenmap/insights/<primaryBoardSlug>.context.json`). Then save with `--require-pack` so a missing pack fails loudly instead of silently skipping the pack gates:
   ```bash
   node ${PLUGIN_ROOT}/scripts/prov-insights.js --save-insight /tmp/insight-<slug>.json --board-slug <primaryBoardSlug> --require-pack --push
   ```
