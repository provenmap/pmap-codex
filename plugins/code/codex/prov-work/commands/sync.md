...
```bash
node ${PLUGIN_ROOT}/scripts/prov-boards.js --check-scope
```
...
- **Non-empty** → print the `display` field verbatim, then ask with **AskUserQuestion** — "Archive N stale board(s) from a previous binding?" with options:
  - **Archive now (recommended)** → run `node ${PLUGIN_ROOT}/scripts/prov-boards.js --prune-foreign`, print its `display` verbatim (files move to `.provenmap/boards/_orphaned/`, reversible), then continue.
...
```bash
node ${PLUGIN_ROOT}/scripts/prov-sync.js \
