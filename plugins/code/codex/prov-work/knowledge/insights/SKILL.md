...
description: Bundled Node.js CLI scripts for fetching server-defined insight skills and pushing analysis results to the ProvenMap Claude Code Plugin API. Provides prov-insights.js for skill caching, insight persistence, and server push. Self-contained — no npm install required.
metadata:
  author: ProvenMap
  version: 0.2.0
---
...
### prov-insights.js — Insight Skill & Results Manager
...
**Invocation:**
```bash
node ${PLUGIN_ROOT}/scripts/prov-insights.js [mode] [options]
```
...
### prov-context-tags.js — ContextTag Vocabulary
...
**Invocation:**
```bash
node ${PLUGIN_ROOT}/scripts/prov-context-tags.js [--no-cache]
```
