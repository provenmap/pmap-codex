 ...
 * When a Bash tool call ran a prov CLI whose JSON output carries a `display`
...
function main() {
  ...
  const command =
    input && input.tool_input && typeof input.tool_input.command === "string"
      ...
      : "";
  if (!command.includes("scripts/prov-")) return;
  ...
  console.log(
    JSON.stringify({
      hookSpecificOutput: {
        ...
        additionalContext:
          "The prov CLI output above includes a `display` field of ready-to-print markdown. Print that display to the user verbatim — do not reformat, reorder, summarise, or rebuild its tables.",
      },
    }),
  );
}
